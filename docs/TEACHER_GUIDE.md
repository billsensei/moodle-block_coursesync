...
activity — is only ever added once. Every standard Moodle question type is
included; a slot naming a question of a type added to Moodle by a plugin, or a random slot whose category could not be rebuilt, is left
out and counted on the results page rather than silently dropped.

**A copied Question bank brings every standard Moodle question type** —
multiple choice, true/false, short answer, matching, essay, numerical,
embedded answers (Cloze), the three drag and drop types (background and drag
images included), select missing words, ordering, random short-answer
matching, description, and the three calculated types (with their variables
and every value set for them). Only a question of a type added to Moodle by
a plugin is left out, and it is named in the results, so you know it did not
come across. A random short-answer matching question picks its
short-answer questions from its own category, and those come across with
it; if it is set to include subcategories, a copied quiz only brings the
subcategories it uses elsewhere, so check it has enough to choose from. Only the current version of each question travels, not
...
from a category; only a type added by a plugin does not. A slot the sync could not rebuild is named on the
